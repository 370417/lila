...
$.fn.multipleSelect = function (this: Cash) {
  const option = arguments[0],
    ...
    methodIsAllowed = (method: string): method is AllowedMethod => (allowedMethods as string[]).includes(method);
  let value;
  ...
  this.each(function (this: EleHack) {
    ...
    if (typeof option === 'string') {
      ...
      data.init();
      const arg = args[1];
      if (typeof arg === 'string' && methodIsAllowed(arg)) {
        value = (data[arg] as any).apply(data, [].slice.call(args, 2));
      }
    }
  });
