...
export default function (d?: typeof deps) {
  function renderResult(ctrl: AnalyseCtrl): VNode[] {
    ...
    } else if (ctrl.study && d) {
      const result = d.findTag(ctrl.study.data.chapter.tags, 'result');
  ...
  const renderAnalyse = (ctrl: AnalyseCtrl, concealOf?: ConcealOf) =>
    h('div.analyse__moves.areplay', [
      ...
      !ctrl.practice && !d?.gbEdit.running(ctrl) ? d?.renderNextChapter(ctrl) : null,
    ]);
  ...
  return function (ctrl: AnalyseCtrl): VNode {
    ...
    const concealOf = makeConcealOf(ctrl),
      ...
      gamebookPlayView = gamebookPlay && d?.gbPlay.render(gamebookPlay),
      gamebookEditView = d?.gbEdit.running(ctrl) ? d?.gbEdit.render(ctrl) : undefined,
      playerBars = d?.renderPlayerBars(ctrl),
      playerStrips = !playerBars && renderPlayerStrips(ctrl),
      gaugeOn = ctrl.showEvalGauge(),
      needsInnerCoords = !!gaugeOn || !!playerBars,
      tour = d?.relayTour(ctrl);
    console.log('d', d?.studyView);
    return h(
      ...
      [
        ...
        study ? d?.studyView.overboard(study) : null,
        tour ||
          h(
            ...
            : h(addChapterId(study, 'div.analyse__tools'), [
                ...(menuIsOpen
                  ...
                  : [
                      ...
                      retroView(ctrl) || d?.practiceView(ctrl) || explorerView(ctrl),
                    ]),
              ])),
        ...
        ctrl.embed || tour
          ...
          : h(
              ...
              study ? d?.studyView.underboard(ctrl) : [inputs(ctrl)]
            ),
        tour ? null : trainingView(ctrl),
        ctrl.embed
          ? null
          : ctrl.studyPractice
          ? d?.studyPracticeView.side(study!)
          : h(
              'aside.analyse__side',
              {
                hook: onInsert(elm => {
        ...
        study && study.relay && d?.relayManager(study.relay),
        ctrl.embed
          ? null
